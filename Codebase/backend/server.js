...
app.get("/api/weather/:destinationId", authenticateToken, async (req, res) => {
  try {
    const destination = destinations.find(d => d.id === parseInt(req.params.destinationId));
    if (!destination) {
  ...
  try {
    ...
    const scoredDestinations = destinationsWithWeather
      .map(destination => {
        ...
        if (destination.currentWeather && 
            WeatherService.matchesPreference(destination.currentWeather.type, userPreferences.weather)) {
          score += 40;
